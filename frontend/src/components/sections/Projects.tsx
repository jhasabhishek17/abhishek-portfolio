...
import { ExternalLink, Github, Star, GitFork, Loader2, Play, Sparkles } from 'lucide-react'
import { featuredProjects } from '@/lib/portfolio-data'
import { getGitHubRepos } from '@/lib/api'
import DevConnectDemo from '@/components/demos/DevConnectDemo'
import ResumeScreenerDemo from '@/components/demos/ResumeScreenerDemo'
import EduPlatformDemo from '@/components/demos/EduPlatformDemo'
...
export default function Projects() {
  ...
  const [activeDemo, setActiveDemo] = useState<'devconnect' | 'screener' | 'eduplatform' | null>(null)

  useEffect(() => { getGitHubRepos().then(d => { setRepos(d||[]); setLoading(false) }) }, [])

  const getDemoType = (title: string) => {
    if (title.includes('DevConnect')) return 'devconnect'
    if (title.includes('Resume')) return 'screener'
    if (title.includes('EduPlatform')) return 'eduplatform'
    return null
  }
  ...
  return (
    <section id="projects" className="py-24">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <p className="mb-10 font-light" style={{color:'var(--muted)'}}>Featured full-stack software and live GitHub repositories.</p>
        ...
        {tab==='featured' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {featuredProjects.map((p,i) => {
              const demoType = getDemoType(p.title)
              return (
                  ...
                  <div className="flex flex-wrap gap-3 pt-2 items-center">
                    {demoType && (
                      <button
                        ...
                        style={{ background: 'linear-gradient(90deg, var(--accent), var(--accent2))', color: '#fff' }}
                      >
                        <Sparkles size={13} /> Interactive Live Demo
                      </button>
                    )}
                    {p.liveUrl&&<a href={p.liveUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs font-mono transition-colors" style={{color:'var(--muted)'}} onMouseEnter={e=>(e.currentTarget.style.color='var(--accent2)')} onMouseLeave={e=>(e.currentTarget.style.color='var(--muted)')}><ExternalLink size={13}/> Live Site</a>}
                    {p.githubUrl&&<a href={p.githubUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs font-mono transition-colors" style={{color:'var(--muted)'}} onMouseEnter={e=>(e.currentTarget.style.color='var(--accent2)')} onMouseLeave={e=>(e.currentTarget.style.color='var(--muted)')}><Github size={13}/> Source Code</a>}
                  </div>
                </div>
              )
            })}
      ...
      {/* Render Modals */}
      {activeDemo === 'devconnect' && <DevConnectDemo onClose={() => setActiveDemo(null)} />}
      {activeDemo === 'screener' && <ResumeScreenerDemo onClose={() => setActiveDemo(null)} />}
      {activeDemo === 'eduplatform' && <EduPlatformDemo onClose={() => setActiveDemo(null)} />}
    </section>
  )
}
